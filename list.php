<?php 

    session_start();
    require_once 'config/db.php';
    if (!isset($_SESSION['user_login'])) {
        $_SESSION['error'] = 'กรุณาเข้าสู่ระบบ';
        header('location: user.php');
    }

    if (isset($_REQUEST['delete_id'])) {
        $id = $_REQUEST['delete_id'];

        $select_stmt = $conn->prepare('SELECT * FROM tbl_order WHERE order_id = :order_id');
        $select_stmt->bindParam(':order_id', $id);
        $select_stmt->execute();
        $row = $select_stmt->fetch(PDO::FETCH_ASSOC);

        // delete an original record from db
        $delete_stmt = $conn->prepare('DELETE FROM tbl_order WHERE order_id = :order_id');
        $delete_stmt->bindParam(':order_id', $id);
        $delete_stmt->execute();

        header("Location: list.php");
    }
?>


<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ร้านเค้กโฮมเมด</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="index1.css">
    <link rel="stylesheet" href="order.css">
    <link rel="stylesheet" href="login.css">
    <style>
        body {
          background-color: burlywood; /* กำหนดสีพื้นหลังเป็น #F0F0F0 */
        }
    </style>
</head>

<body>
    <div class="background">
        <!-- home -->
        <nav>
            <div class="container">
                <div class="nav-con">
                    <div class="logo">
                        <a href="">ร้าน ขะ-หนม-ตุ้น</a>
                    </div>
                    <ul class="menu">
                        <li><a href="#"></a></li>
                        <li><a href="#"></a></li>
                        <?php 
                        
                            if (isset($_SESSION['user_login'])) {
                                $user_id = $_SESSION['user_login'];
                                $stmt = $conn->query("SELECT * FROM users WHERE id = $user_id");
                                $stmt->execute();
                                $row = $stmt->fetch(PDO::FETCH_ASSOC);
                            }
                        
                        ?>
                        <li><a href="">ID ลูกค้า: <?php echo $row['id'] ?> </a></li>
                        <li><a href="">ลูกค้า: <?php echo $row['firstname'] . ' ' . $row['lastname'] ?> </a></li>
                        <a href="usertest.php" class="btn btn-primary" id="">รายการสินค้า</a>
                    </ul>
                </div>
            </div>
        </nav>
        <br>

        <div class="container">
            <h2 class="text-center">ตะกร้าสินค้า</h2>
            <div align="right">
                <a href="addlist.php" class="btn btn-success">เพิ่มสินค้า</a>
            </div><br>
                    <table class="table table-striped table-bordered table-hover">
                        <thead>
                            <tr>
                                <!-- <td>ID: ลูกค้า</td> -->
                                <th style="width: 300px;">ชื่อสินค้า</th>
                                <th>ท็อปปิ้ง</th>
                                <th>ราคา</th>
                                <th style="width: 250px;">รายละเอียด</th>
                                <td style="width: 90px;"></td>
                                <td style="width: 90px;"></td>
                            </tr>
                        </thead>
                        <tbody>
                            <?php 
                                $select_stmt = $conn->prepare("SELECT * FROM tbl_order WHERE id = $user_id");
                                $select_stmt->execute();

                                while ($row = $select_stmt->fetch(PDO::FETCH_ASSOC)) {
                            ?>
                                <tr>
                                    <!-- <td><?php echo $row['id']; ?></td> -->
                                    <td><?php echo $row['cake_name']; ?></td>
                                    <td><?php echo $row['cake_top']; ?></td>                       
                                    <td><?php echo $row['cake_price']; ?></td>
                                    <td><?php echo $row['cake_details']; ?></td>                         
                                    <td class="text-center"><a href="edit_list_order.php?update_id=<?php echo $row['order_id']; ?>" class="btn btn-warning">แก้ไข</a></td>
                                    <td class="text-center"><a href="?delete_id=<?php echo $row['order_id']; ?>" class="btn btn-danger">ลบ</a></td>
                                </tr>
                            <?php } ?>
                        </tbody>
                    </table>

            <div align="right">
                <a href="checkout.php" class="btn btn-primary">ยืนยัน ทำรายการสั่ง</a>
            </div><br>
        </div>

        <div class="container">
        <div class="d-flex flex-column flex-sm-row justify-content-between py-4 my-4 border-top">
        
            <p>
            ช่องทางการติดต่อ :<br>    
            © เบอร์โทร 092-796-5160 <br>
            @ Facebook : Natthaphon niamnuam <br>
            @ LINE : Natthaphon niamnuam</p>   
            <ul class="list-unstyled d-flex">
                <li class="ms-3"><p></p></a></li>
                <li class="ms-3"><p></p></a></li>
            </ul>
        </div>
    </div>
    </div>
    </div>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>

</body>

</html>